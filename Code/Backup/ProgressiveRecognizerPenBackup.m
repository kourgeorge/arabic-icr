...
inner_Seq = Sequence;
set(findobj('Tag','TEXT'),'String','Wait...');
drawnow();

ClosestWPs = RecognizeWPkdTree( Sequence, [folder,'\kdTree\Angular.mat'], 1, kNN );

len = size(ClosestWPs,1);
str = '';

for i=1:len
    str = [str,'  ',ClosestWPs{i}];
end
set(findobj('Tag','TEXT'),'String',['The closest WPs are: ' str]);
