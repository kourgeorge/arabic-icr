...
This paper proposes an efficient Arabic handwritten characters recognizer aimed at facilitating real-time handwritten script analysis tasks such as segmentation.
...
A well known technique for achieving a speed-up is by embedding the compared objects into a space equipped with an efficient metric which approximate the distance function in the original space.
...
The real-time nature of the segmentation technique in the mentioned work, strictly required the characters classifier to be extremely fast.
In this paper, we present the character classifier that was used in a real-time recognition-based Arabic script segmentation technique described in \cite{kour2014real}.
...
\label{sec:approach}
In the presented work, we propose a fast and accurate classification technique for hamdwritten Arabic characters.
...
The embedding of the sample set into a normed space facilitates the usage of metric indexing and approximate k-NN techniques such as k-d trees \cite{bentley1975multidimensional} and Locality Sensitive Hashing (LSH) \cite{gionis1999similarity}.
...
For instance, when we employed the shape context descriptor, the embedding produced sparse vectors in $\mathbf{R}^{2422}$. 
...
Such composite characters are usually difficult to be separated by the segmentation system and are usually treated as a single letter.
In this work we considered the following combinations: \RL{l.h-} , \RL{lm-} and \RL{lA}.
...
When the case when the top 3 candidates is considered, the difference in accuracy is small between the first and second configurations, and the latency resulted from the re-scoring done in the first configuration, in most cases, will not pay. 
The dimensionality reduction and the indexing (i.e., building the k-d tree) processes are both computationally expensive and time consuming. 
While both stages are usually performed off-line, in systems where the learning is performed on-line, the latency they impose on the learning process is undesired.
...
Skipping the dimensionality reduction and the indexing stages has decreased the training time of the system in about 7 seconds in each iteration of the cross validation.
...
Although we noted that the vast majority of the delay was caused by the dimensionality reduction stage, using k-d tree in the Fast Learning configuration would negatively affect the classification repose time, due to the sensitivity of the k-d tree to high dimensional data.
...
The goal of the following experiment, shown in Table \ref{table:features_comparison}, is to evaluate the  the effectiveness of the different shape descriptors.
\begin{table}
\centering
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{ | c | c | c |}
...
\section{Using The Classification Information for WPs recognition}
...
We have proposed several interesting activation configurations of the proposed approach aiming at finding the desired balance between accuracy and response time.
