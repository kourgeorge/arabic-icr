...
The proposed approach goes through three stages as can be seen in Figure \ref{fig:system_flow}.
The first stage is composed of two steps.
In the first step, \emph{points of interest} (POIs), are continuously nominated while the stroke is being scribed using a \emph{horizontal points} (HF) identification process.
In the second step, once a POI is found, the candidates matrix is update with the classification information of sub-strokes imposed by the identified POIs.
...
While false positive SPs can be easily removed, missed SPs cannot be easily recovered. 
...
\begin{figure}
     \begin{center}
        ...
        \subfloat[sequence 2]{
           \includegraphics[width=0.3\textwidth]{./figures/angles_2}
        }        
    \end{center}
...
We generate an upper triangular scoring matrix $D\in\mathbb{R}^{(L+1)\times (L+1)}$ where each cell $D_{i,j}$ represents the sub-stroke $S_i^j$. 
It contains the classification information and scoring, for the sub-strokes $S_i^j$, returned by the Arabic characters classification system described in Chapter \ref{chap:characters_classification}. 
...
Using the relative location of the sub-stroke we restrict the position options the given sub-stroke can present and avoid activating the recognition process with all four letter position possibilities.
...
In order to determine that a given SP resides inside a loop we have employed a Matlab package that includes an implementation for poly-line intersection algorithm \cite{legland2014geom2d}.
...
This extension may look very similar to the letter \RL{-a} during the stroke scribing; and thus under-scored by the recognition system, and eventually, result in over-segmenting the letter \RL{d}. 
...
\caption{Over-segmentation appeared frequently in the letter \RL{-m}, since it contains two horizontal segments.}
