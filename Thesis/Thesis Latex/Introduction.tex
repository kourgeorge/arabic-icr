...
Writing, which has made much of the culture and civilization possible, was developed long time ago as a mean to expand human memory and to facilitate communication. 
Despite the long standing prediction that digital computers will challenge its future, handwriting persists. 
Nowadays, handwriting remains a commonly used mean for communication and recording of information in the daily life, and therefore, a growing interest in the \emph{handwriting character recognition} (HWR) field has emerged in recent years, and has now been a topic of research for over four decades .
...
Each script has its own rules on how letters are combined to form shapes that represent higher level linguistic units \cite{plamondon2000online}.
...
Its unconstrained nature that produces a huge variety in the handwriting of different people, makes the recognition of Arabic script extremely difficult.
...
\begin{figure}
    ...
    \caption{On-line versus Off-line word \cite{plamondon2000online}.}
...
In general, off-line HWR systems are less accurate than on-line systems, but, they are now good enough that they have a significant economic impact on for specialized domains such as interpreting handwritten postal addresses on envelopes and reading courtesy amounts on bank checks \cite{melin2007analysis}.

\iftoggle{edit-mode}{\hspace{0pt}\marginpar{A general flow for HWR}}{}
There is a large variety of approaches for HWR, and one may find it difficult to draw a general flow which is valid for all existing systems. 
However, there are several stages that can be found in most HWR methods.
HWR systems usually consist of several subsequent stages and here we will mention the most common.
...
The samples go through preprocessing stage that include filtering of noises, re-sampling the stroke information to obtain equidistant stroke, and normalized to a standard size. Additional preprocessing may include slant and slope corrections. 
...
Their different nature imposes dissimilar challenges and thus variable level of effort is required to be spent on the various stages.
The off-line HWR systems usually consist of an additional steps of layout analysis and text lines extraction, which are activated at the beginning of the preprocessing stage. 
...
There are no established definitions for the lexicon size. 
...
\end{compactitem}
Open-vocabulary tasks refer to the recognition of any words without the constraint of being in a dictionary.
...
Using the holistic approach may be possible for small vocabulary of words, however, this is not feasible for large vocabularies (20,000 words or more). 
...
Nevertheless, some of those languages has latterly adopted the Latin characters, but in general, people can still read the Arabic script \cite{zeki2011segmentation}.
...
It has only started in 1975 by Nazif \cite{nazif1975system}, while the earlier research efforts in Latin may be traced back to the middle of the 1940s.
...
The Arabic script consists of 28 basic letters and is written from right to left in a semi-cursive manner in both printed and handwritten.
...
The parts of the word that are graphically connected are called \emph{word-parts} (WP). 
...
Ignoring the delayed strokes reduced the number of different WP to 40,000 \cite{saabni2009efficient}. 
...
%For instance, the main body of the word part \RL{byt} /bayt/, is usually written in a single stroke however, sometimes it may be written by some writers using 3 strokes.
%We have also considered the common combination of the letter \RL{l} followed by the vowel \RL{A} as a single letter which is commonly drawn as \RL{lA} /la/. 
%Also, three consequent appearances of the letter \RL{-b-} /b/ in the middle of the word-part looks as follows: \RL{-b-y-t-}. 
...
An integral part of the hand writing recognition process, when the analytic approach is considered, is segmentation, and therefore, it has long been a critical area of the OCR process. 
...
Segmentation and recognition of cursive text are two tasks intertwined and dependent on each other. It is believed that wrong segmentation will often results in major contribution to the error of the recognition algorithm \cite{brodowska2011oversegmentation}. But in order to determine the segmentation, the system may try to seek a pattern that will math a member of the system's alphabet \cite{casey1996survey}. 
This paradoxical condition is called ''the segmentation catch 22''.

\iftoggle{edit-mode}{\hspace{0pt}\marginpar{The context dependent of segmentation}}{}
Reliable recognition system requires more than a good matching to a library of letters symbols.
The segmentation decision is not a local decision, independent of previous and subsequent decisions. 
That is, a poor match on a later pattern can cast doubt on the correctness of the current segmentation/recognition result. 
...
For example, the letter sequence "cl" can often closely resemble a "d", but usually such a choice will not constitute a contextually valid result.

\iftoggle{edit-mode}{\hspace{0pt}\marginpar{The context dependent of segmentation}}{}
Several segmentation approaches have been proposed in the literature for Arabic OCR, yet, correct and efficient segmentation of Arabic text is not easily achievable and considered to be a challenging problem even for printed text. 
A common approach that is followed by many researchers is over-segmentation of the text and validating each such candidate segmentation point by extracting feature vectors representing the segmented parts to some classifier or rules based engine \cite{daifallah2009recognition}.

Methods used by segmentation-based approaches can be mainly classified into two: Dissection and Recognition-Based Segmentation. 
Dissection techniques learn the characteristic of the segmentation point and attempt to find appropriate candidate points. 
For instance, local minima in the upper or lower contour is commonly used for segmenting English cursive script. 
Other widely used feature that characterizes a segmentation point is the low slope of its local environment. 
The dissection of words into segments do not necessarily correspond to exactly one character per segment, however it could segment the words into components called graphemes, which are a combination of two or three letters, or a part of a letter. 
The relationship between graphemes and letters is applied in a later phase. 
The recognition-based techniques operate quite differently. 
The method includes using a moving window with a predefined width which breaks the word into many overlapping pieces without regard to its content and the attempts to classify those sub-components to find points of interest or trajectories. 
An iterative or parallel scanning of the words followed by a recognition method is used to search for "satisfactory" classification scoring for joint sub-components usually by generating a lattice of all or many possible combinations. 
The final decision is determined by the best path through the lattice. While avoiding using complex dissection methods, such techniques rely heavily on the classifier accuracy \cite{casey1996survey}. \\
...
The classical approach, which is usually named \emph{dissection}, attempts are made to segment the text into character using "character-like" properties. 
...
A throughout survey on character segmentation is given in \cite{casey1996survey} and \cite{brodowska2011oversegmentation}.  
