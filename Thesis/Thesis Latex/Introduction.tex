...
In the case of \emph{supervised learning}, the classification is done based on a dataset of data containing labelled samples, referred to as the training set, as given in Definition \ref{def:supervised_learning}.
\begin{definition}
Given a domain space $X$, a target space $Y$ and a training set $S=\{(x_i,y_i)\}_{i=1}^{m}$ where $x_1,x_2,..,x_n\in X$ and $y_1,y_2,...,y_n \in Y$. 
Let us assume that there is an unknown target function $f$ for which $f : X\rightarrow Y$. 
A Learning algorithm $A$ produces hypothesis function $h$  based on $S$ such that $h \approx f$. 
...
Given a classification problem, it is reasonable to try looking for a space to which the samples can be transformed and which can be portioned into regions that represent the different categories. 
Accordingly, the problem of classification basically becomes the problem of portioning the space into regions. 
Ideally, the partitioning should be performed such that none of the decisions are ever wrong. 
When such partition cannot be found, one would like to minimize the probability of error.
...
A review of the supervised classification algorithms is given by Mohamed Aly in \cite{aly2005survey}. 
...
Characters, words and sentences are modelled using HMM and classified using the solution to its learning problem.
The interested reader unfamiliar with HMM, can refer the following two references \cite{kadous2002temporal, shu1996line}, for a well-written introduction.
...
First, the HMM model makes some powerful assumption about the data that may not necessarily be true, such that the Markovian assumption that presumes that the transition probabilities depend only on the current state. 
Second, there is no specific way to determine the number of states requiring try and error.
