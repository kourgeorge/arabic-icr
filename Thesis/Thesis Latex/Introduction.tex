...
There are different types of problems with varying complexity within HWR.
...
\emph{TODO: State of the Art HWR - take that from the book.}
...
\iftoggle{edit-mode}{\hspace{0pt}\marginpar{The research on HWR in other languages}}{}
\emph{The research on HWR in other languages}
...
The samples go through a preprocessing stage that includes filtering of noises, re-sampling the stroke information to obtain equidistant samples, and normalized to a standard size. 
...
Finally, using a classification algorithm the basic units are labeled. 
...
The Arabic script is a descendant of the Aramaic script. 
...
It is one of the five most common languages in the world and one of the six United Nations official languages since 1974 \cite{burrow2004arabic}. 
...
The use of Arabic language extended in the $7^{th}$ and $8^{th}$ centuries from India to the Atlantic ocean due to the Islamic conquests \cite{saabni2009efficient}. 
...
Obligatory dots and strokes used in the Arabic script are called \emph{delayed strokes} and are usually drawn last in the when scribing a WP or a word. 
...
See table of identical rasm Arabic letters in Figure \ref{fig:same_body_letters}.
...
The main body of most Arabic letters are written by a single stroke. 
A single stroke can contain a single or multiple letters.
...
Consecutive appearances of the letter \RL{-b-} look very similar to the \RL{-s-} /s/, the only to distinguish between the two options is by looking at the additional strokes.
...
Characters classification is a complex and challenging problems in the field of pattern recognition.
This is due to the variation in characters shapes resulted from differences in writing habits, styles, social and educational levels of the writer.
...
The $k$-NN algorithm is a well-known classification technique in supervised learning. 
If $k=1$, then the object is simply assigned to the class of that single nearest neighbour.
When $k>1$, the object being assigned to the class most common amongst its $k$ nearest neighbours
...
First, the $k$-NN algorithm needs a large dataset in-order to achieve high classification accuracy. 
...
The segmentation decision is not a local decision because it may affect subsequent segmentation decisions. 
Thus, a poor match of the current segment to some class in the letter library can cast doubt on the correctness of the future segmentation decisions.
However, even a series of satisfactory pattern matches can be judged incorrect if contextual requirements of the system output are not satisfied \cite{casey1996survey}.
...
A real-time, recognition-based, segmentation technique is detailed in Chapter \ref{chap:strokes_segmentation}.
We present a procedure for nominating segmentation points whilst the stroke is being scribed and propose algorithms for selecting the final segmentation set of segmentation points.
The results obtained by the classification system and the segmentation process are presented in the corresponding chapters.
...
A paper titled \emph{"Fast Classification of Handwritten On-line Arabic Characters"} was accepted for publication in the proceedings of the \emph{6th International Conference of Soft Computing and Pattern Recognition} (SoCPaR 2014) in June 20, 2014.

Additional conference paper titled \emph{"Real-time Segmentation of On-line Handwritten Arabic Script"} will be presented in the \emph{14th International Conference on Frontiers in Handwriting Recognition} (ICFHR-2014) in April 21, 2014 and will be published in the conference proceedings.
