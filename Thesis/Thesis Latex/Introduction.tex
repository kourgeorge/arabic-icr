...
However, in this study, we question the necessity of the requirement by demonstrating the feasibility of determining the position of the segmentation points (SPs) while the stroke is being written; and by doing so, accelerating the recognition process.
The proposed segmentation method is performed in the strokes level enabling analysis task related to segmentation and recognition of the stroke into letters to be done while the stroke is being written. 
The proposed segmentation technique is detailed in Chapter \ref{chap:strokes_segmentation}.
We demonstrate how segmentation points are nominated while the stroke is being scribed, aplly rules based filtering engine and propose several algorithms for selecting the final segmentation set of segmentation points.

Its real-time nature of the recognition-based segmentation method required the development and employing an extremely fast and accurate characters classifier that employs state of the Art similarity measure and search techniques for fast classification of characters bast on nearest neighbours.
The method include a pre-reprocessing task; compared feature extraction techniques are discussed; and an embedding to the wavelet domain is exploited in order to enable fast classification using nearest neighbours classification. 
The Arabic Letters classifier is detailed in Chapter \ref{chap:characters_classification} and a brief explanation on wavelets and the wavelet transform is given in Appendix \ref{app:wavelets}.

The results obtained by the classification system and the segmentation process are given in Chapter \ref{chap:results}.
 
A summary and future work are proposed in Chapter \ref{chap:summary}. 

Another main contribution of this work is the creation of Arabic characters database extracted from a de-facto a standard database for on-line Arabic Handwritten script which provides a large sample set of handwritten words written, named ADAB. 
The manual segmentation process performed on the ADAB database in order to obtain letters samples are given in Appendix \ref{app:data_collection}. 

The system has several assumptions that are not always true when considering Arabic handwriting. 
First, it does not regard additional strokes.
However, it was shown that using this information can be used to improve the classification results.
Second, since the classification is performed independently in the stroke level, it is assumed that letters cannot be written in more than one stroke, which is a very reasonable assumption but not always true. 
