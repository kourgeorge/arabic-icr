...
Writing has made much of culture and civilization possible.
It was developed as a mean to expand human memory and to facilitate communication. 
Converting handwritten script into its digital analogue is highly motivated by the ease and convenience of the digital representation.
...
Although considered a well developed technological field, HWR remains an area of active scientific research and creative engineering \cite{borovikov2004survey}.
...
At one extreme there is the case of isolated characters written inside graphical boxes in which the segmentation problem is already solved. 
At the opposite extreme there is the case of cursive and unrestricted handwriting in which words or portions of a word are written with a single stroke using ligatures that connect adjacent letters. 
...
Figure \ref{fig:ha_different} shows several handwritten samples the letter \RL{.h}, in its isolated form, written on a digitizer by several writers.
...
The field of HWR can be divided in several ways, however, the most common categorization is the one that distinguishes between \emph{off-line} (also called \emph{static}) and \emph{on-line} (also called \emph{dynamic}).
Off-line HWR focuses on documents that have been written on paper at some previous point of time. 
A digital image of the document, represented as a raw two-dimensional pixel data, is fed to the computer and the system attempts to convert the spatial representation of the letters into digital symbols \cite{al2011online}. 
...
\begin{figure}
    ...
    \caption{Off-line (\ref{fig:offline}) vs. on-line (\ref{fig:online}) representation of a word \cite{plamondon2000online}.}
...
While having much in common, approaches for on-line and off-line HWR are different due to the disparity in the input data representation. 
Their different nature imposes different challenges and thus variable levels of efforts are required to be invested on the various stages.
...
The research on on-line HWR started in the 1960's and has been receiving a great interest from the 1980's \cite{tagougui2013online}.
...
\subsection{Holistic versus Analytic Handwriting Recognition}
...
However, a lexicon that contains tens of words is considered a small lexicon and a lexicon that contains tens of thousands of words is considered a very large lexicon.
Open-vocabulary systems refer to such that attempt to recognize any word without the constraint of being in a dictionary.
The lexicon is a key-point post-processing stage in many systems which exploit linguistic knowledge to filter out many possible options that are not included in the lexicon, and consequently raising the recognition rate.
...
The limitation of this approach is that the classifier needs to be trained over all words in the dictionary and to maintain a models for each word.
...
A survey done in \cite{al2011online} on Arabic HWR found that system based on the analytic approach, in general, achieve higher recognition rates than the holistic systems in cases where words are written in cursive manner, such in the Arabic script. 
...
Arabic is spoken, as their first language, by more than 350 million people around the world.
It is an official language in 27 states, the third most after English and French.
...
Although Arabic is used mainly in the Arab countries, which consists of about 5.5\% of the world population, almost all Muslims, around 25\% of the world population can read Arabic script to a certain degree as it is the language of the Holy Qur'an \cite{zeki2011segmentation}.
...
Most letters are written in four different letter shapes depending on their position in the word, e.g., the letter \RL{`} appears as \RL{`}  in its isolated form, \RL{`-} in its initial form, \RL{-`-} in its medial form and \RL{-`} in its final form. 
Among the basic letters, six are dis-connective - \RL{A}, \RL{d}, \RL{_d}, \RL{r}  \RL{z} and \RL{w}. 
...
The i'jam are the pointing diacritics added to the main body of the letter, called \emph{rasm} (\RL{rsm}), and their role is to distinguish between various constants, such as the medial form letters \RL{-b-}, \RL{-t-}, \RL{-_t-}, \RL{-n-}, \RL{-y-}.
...
In addition to the frequently used dots, the \RL{'} (hamza) is another type of i'jam that distinguish between the letters \RL{k} and \RL{l} in their isolated and final forms.
...
\hline
\end{tabular}
\caption{List of Arabic characters with identical rasm.}
...
These additional strokes were added around the $7^{th}$ century, to prevent the Qur'an from being misread by Muslims \cite{burrow2004arabic}.
...
Saabni and El-sana \cite{saabni2009efficient} have explored a large collection of Arabic texts and extracted about $300,000$ different word combinations made of more than $82,000$ different WPs.
Ignoring the delayed strokes, the number of different WP had reduced to about $40,000$.
...
First, while the main body of most Arabic letters are written within a single stroke, there are some letters that usually written using two strokes, such as the letter \RL{-k-} (the middle form of the letter \RL{k}). 
...
Second, different writers may write the main body of the same WP in a different number of strokes.
For example, rasm of the word \RL{byt} can be written in a single, two or three strokes.
Third, consecutive appearances of the letter \RL{-b-} look very similar to the \RL{-s-}, the only to distinguish between the two options is by looking at the additional strokes.
...
Many techniques were investigated for classifying Arabic characters, including \emph{Artificial Neural Networks} (ANN) \cite{alijla2012oiahcr}, \emph{Decision Trees} \cite{ismail2012online}, \emph{$k$ Nearest Neighbours} ($k$-NN) \cite{elglaly2011isolated} and \emph{Hidden Markov Models} (HMMs). 
...
While having many advantages, HMMs make some powerful assumption about the data that may not necessarily true, such as the Markovian assumption which presumes that the transition probabilities depend only on the current state \cite{kadous2002temporal}. 
...
Segmentation is an integral part of the HWR process, especially in the analytic approach.
...
Several segmentation approaches have been proposed in the literature, yet, correct and efficient segmentation of the Arabic text is not easily achievable and considered to be a challenging problem even for printed text.
...
However, even a series of satisfactory letters matches can be judged incorrectly if contextual requirements of the system output are not satisfied \cite{casey1996survey}.
...
In Arabic, as mentioned before, two or three consequent appearances of the rasm \RL{-b-}, that is common for the letters \RL{-y-}, \RL{-t-} and more, are very similar to the letter \RL{-s-}, and the only way to distinguish between them is by considering the delayed strokes and the contextual validity.
...
Dissection techniques attempt to find appropriate candidate points by learning the characteristic of the \emph{segmentation point} (SP) or by using a static rules. 
...
A common approach, followed by many researchers, is to over-segment the text at first, i.e, finding some set of potential splitting points that partition the handwritten word into primitives, and then to eliminate improper candidates point \cite{daifallah2009recognition}.
...
The performance of the system was compared to the classification results obtained using ANNs and decision trees. 
...
Randa et al. \cite{elanwar2012unconstrained} proposed a two stage on-line Arabic handwritten text segmentation system based on HMMs. 
...
A conference paper titled \textbf{\emph{"Fast Classification of Handwritten On-line Arabic Characters"}} was published in the proceedings of the \emph{6th International Conference of Soft Computing and Pattern Recognition} (SoCPaR 2014).
The paper discusses the character classification system presented in Chapter \ref{chap:characters_classification}.


Additional conference paper titled \textbf{\emph{"Real-time Segmentation of On-line Handwritten Arabic Script"}} was presented in the \emph{14th International Conference on Frontiers in Handwriting Recognition} (ICFHR 2014). 
