...
The Data, it's quality and quantity, are fundamental part of any supervised machine learning technique.
...
Even less datasets were created for the on-line Arabic script. 
...
In this work, we have used the only publicly available corpus of on-line Arabic handwriting, called the \emph{ADAB} (Arabic DAtaBase) database, which is a de-facto a standard in this field.
It was developed to advance the research and development of Arabic on-line handwritten text recognition systems and consists of more than 20k Arabic handwritten words scribed by more than 170 different writers \cite{el2011line}.
The samples are words that were taken from the 937 Tunisian town/village names.
Each stroke, represented as a node in the XML, contains the sequence of $(x,y)$ coordinates of the pen position expands from the pen-down to the corresponding pen-up event, as can be seen in Figure \ref{fig:adab_inkml}.
...
To facilitate the database construction, we have created a user friendly system that reads the samples from the ADAB database, display the different strokes of a sample in different color, and enable a human expert to easily mark the segmentation points on the strokes.
