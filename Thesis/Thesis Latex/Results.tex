...
\begin{tabular}{ | c | c | c | c |}
  \hline
  Experiment & CCR [Top 1]  & CCR [Top 3] & CCR [Top 5]  & Time [ms]\\
  \hline                 
  Baseline & 90.4\% & 95.77\% & 96.72\% & 28, 23, 24 \\
  \hline
  No DR & 90.97\% & 95.93\% & 97\% & 40, 38,39\\ 
  \hline
  No embedding & 90.66\% &  94.23\% & 95\% & 30,27,28\\
  \hline
\end{tabular}
\caption{CCR and time performance of the different configurations. (with scoring)}
...
\begin{tabular}{ | c | c | c | c |}
  ...
  Experiment  & CCR [Top 1]  & CCR [Top 3] & CCR [Top 5]  & Time [ms]\\
  \hline                 
  Baseline  & 86\% & 93\% & 95.54\% & 0.1,0.18,0.18  \\
  \hline
  No DR & \% & \% & \% & \\ 
  \hline
  No embedding & 89.61\% &  93.59 \% & 94.81\% & 0.68,0.69,0.72 \\
  \hline
\end{tabular}
\caption{CCR and time performance of the different configurations. (No Scoring)}
\label{table:dr_embedding_comparison} 
\end{table}
...
\begin{tabular}{ | c | c | c | c |}
\hline
	Shape Descriptor  & CCR [Top 1]  & CCR [Top 3] & CCR [Top 5] \\
	\hline 
	Shape Context & 86\% & 95\% & \\                
  	\hline
  	MAD & 88.5\% & 78\% &  \\
...
\label{subsec:candidates_scoring}
...
In this section we asses the effectiveness and efficiency of applying additional scoring stage that consists of expensive similarity measure techniques to precisely asses the perceptual similarity between the given unlabelled sample and the candidates and rank the candidates.

The results of the letters classification accuracy and performance can be seen in Table \ref{table:scoring_Classification}.
...
\hline
  \multirow{2}{*}{$\|E(c_i)-E(q)\|_1$} & Recognition Rate & 86\% & 93\% & 95.6\% \\\cline{2-5} 
                                       & Overall Time [ms] & 0.13 & 0.16 & 0.14 \\
  \hline                 
  \multirow{2}{*}{$DTW(c_i,q)$} & Recognition Rate & 91\% & 96\% & 97\% \\\cline{2-5}
                                & Overall Time [ms] & 24 & 24 &  24 \\
  \hline
  \end{tabular}
\caption{Scoring result}
\label{table:scoring_Classification} 
\end{table}
...
It can be seen that applying DTW on the ten candidates decreased the performance of the classification in more than 20 times, and the accuracy of the system did not improve in more than 5\%.

\begin{table}
\centering
\begin{tabular}{ | c | c | c | c |}
\hline
Scoring Formula & SR & RR & Time [ms]\\
\hline
  $\|E(c_i)-E(q)\|_1$ & 76\% & 70\% & 0.6 \\ 
  \hline
  $DTW(c_i,q)$ & 82.7\% & 70\% & 0.6  \\
  \hline
  \end{tabular}
\caption{Scoring result}
