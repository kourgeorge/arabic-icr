...
\item Talk about the different classification techniques that is common in handwriting recognition such as NN, HMM, neural networks, basyian classification, and linear classification or discrimination.
\item for general overview see M. P.-H. Wu, “Handwritten character Recognition. Thesis”
\item explain why did we choose to use NN rather than HMM.
...
\begin{itemize}
\item What is EMD?
\item Where it was used and for what?
\item Was it used for handwriting recognition? Where?
\item Was it used for on-line handwriting recognition?
\end{itemize}

\emph{Earth Movers Distance} (EMD) has experimentally verified to capture well the perceptual notion of a difference between images and shapes \cite{grauman2004fast}. It is widely used in content-based image retrieval to compute distances between the color histograms of two digital images.

EMD is basically a measure of the dissimilarity between two histograms. Descriptively, if the histograms are interpreted as two different ways of piling up a certain amount of dirt, the EMD is the minimal cost of turning one pile to other, where the cost is assumed to be the amount of dirt moved times the distance by which it moved.

EMD naturally extends the notion of a distance between single elements to that of a distance between sets, or distributions, of elements. When used to compare distributions with the same overall mass, EMD is a metric.

As seen in previous sections, Both features used in this thesis produces histograms. The implementation used in this work for the SC produces a feature vector with a constant total mass, i.e. . Thus, EMD is the best fit for our needs.
In order to find the perceptual distance between the two sequences
we should find the dissimilarity measures between two feature histograms. 

It was also used by Saabne in cite{[]}for off-line handwriting recognition. However, for the best of our knowledge, this is the first use of EMD in the on-line handwriting recognition.  

{\color{blue}Crossbin distance measures take into account the fact that histograms are based in fea- ture space and it is possible for histogram mass to move between bins in feature space. They penalize this move- ment according to the distance covered, called the ground distance. The earth mover's distance (EMD) is a natural and intuitive metric between histograms if we think of them as piles of sand sitting on the ground (feature space). Each grain of sand is an observed sample. To quantify the differ- ence between two distributions, we can measure howfar the grains of sand have to be moved so that the two distributions coincide exactly. EMD is the minimal total ground distance travelled weighted by the amount of sand moved (called flow)\cite{shirdhonkar2008approximate}}

\begin{definition}
Given two histograms $f_1(i), f_2(i)$, a ground distance $d_{ij} \geq 0$ and a flow $g_{ij} \geq 0$ (from bin $i$ to bin $j$). EMD is defined as follows:
\begin{equation}
EMD(f_1,f_2)=\min{\frac{\sum\limits_{i,j} g_{ij}d_{ij}}{\sum\limits_{i,j} g_{ij}}}
\end{equation} 
such that $\sum\limits_{j} g_{ij} \leq f_1(i)$, $\sum\limits_{i} g_{ij} \leq f_2(i)$, $\sum\limits_{i,j} g_{ij}=min\left(\sum\limits_{i} f_1(i), \sum\limits_{i} f_2(i)\right)$
\end{definition}

Computing EMD is based on a solution to the well-known transportation problem. It can be computed as the minimal value of a linear program.

\subsection{EMD Embedding}

{\color{blue}The minimum cost of matching features from one shape to the features of another often reveals how similar the two shapes are. Unfortunately, computing the optimal matching for a single shape comparison has a complexity that is superpolynomial
...
The major hurdle to using EMD is its $O\left( {{N^3}\log N} \right)$ computational complexity (for an N-bin histogram).Various approximation algorithms have been proposed to speed up the computation of EMD. Piotr Indyk has suggested an embedding technique in which the un-normed EMD metric is embedded into a normed space, so that the distance between the two images is comparable to the distance between the two points which represent the embedding of the two images.
...
\subsection{Approximate EMD Embedding}

\begin{itemize}
\item Is on-line handwritten data can be approached as time series?
\item formal problem definition
\item define the vector space. it is a time series but we do not use the time value but only its index since we remove the timing issue in the preprocessing phase.
\item Talk about the issue that vectors in our vector space may have various lengthes.
\item important properties similarity measure techniques
\item mention similarity measure like DTW and EMD.
\end{itemize}

mention DTW, EMD and ERP.

\emph{TODO: see \cite{hetland2009basic} for a good introduction to similarity search}
...
\section{Similar objects retrieval}

Mention the following:
\begin{itemize}
\item The importance of the triangular equality in indexing.
\item Mention performance.
\item fast retrieval and indexing techniques (section). lower bounding and kd-tree.
\item what is the difference between lower bounding and indexing. see \cite{chen2005similarity}.
\item Why do work on the feature vector and not on the trajectory sequence.
\item why don't we use the euclidean norm in the feature space?
\item Implementation Details.
\end{itemize}

Indexing is any form of preprocessing of the data set designed to facilitate efficient retrieval.
