...
\emph{Earth Movers Distance} (EMD), introduced by Rubner et al. in \cite{rubner2000earth}, has experimentally verified to capture well perceptual notion of a difference between images and shapes \cite{grauman2004fast}. It is widely used in content-based image retrieval to compute distances between the color histograms of two digital images \emph{TODO: and citation}. 
...
\begin{equation}
EMD(f_1,f_2)=\min{\frac{\sum_{i,j} g_{ij}d_{ij}}{\sum_{i,j} g_{ij}}}
\end{equation} 
such that $\sum_{j} g_{ij} \leq f_1(i)$, $\sum_{i} g_{ij} \leq f_2(i)$, $\sum_{i,j} g_{ij}=min\left(\sum_{i} f_1(i), \sum_{i} f_2(i)\right)$
\end{definition}
...
\item see \cite{hetland2009basic} for a good introduction to similarity search
\item do we need a normed space to do indexing and pruning?
\end{itemize}
...
For the user convience and for the completness, first we would like to give the definition of \emph{normed vector space}.

\begin{definition}
Given a vector space $D$ over field $F$ the pair $(D,\|\cdot\|)$ is a \emph{normed space}, if for all $x,y\in D$ and $\alpha \in F$ the following conditions hold:
\begin{itemize}
...
In this chapter we will consider some embedding techniques that allow us to get the best of the both world. It will allow us to approximate 

Being not a metric prevent us to perform indexing and pruning.

that not always always metrics, we would like to tra
...
{\color{blue}The embedding of EMD given in \cite{indyk2003fast} provides a way to map weighted point sets A and B from the metric space into the normed space L1, such that the L1 distance between the resulting embedded vectors is comparable to the EMD distance
between A and B themselves. Working in a normed space is desirable since fast approximate NN search techniques such as
LSH require it. The general idea of the embedding is to compute and concatenate several weighted histograms of decreasing
resolution for a given point set. Formally, given two point sets A and B, each of cardinality n, and each containing points in
$R^d$: impose grids $G_i$, $-1 \leq i \leq log(\Delta)$, on the space $R^d$, with grid $G_i$ having side length $2^i$, and $\Delta$ equal to the diameter of $A \bigcup B$. Each grid is translated by a vector chosen randomly from $[0, \Delta]^d$. To embed point set A, for each $G_i$ create vector $v_i$ with one coordinate per grid cell, where each coordinate counts the number of points in the corresponding cell, i.e., each $v_i$ forms a histogram of A. The embedding of A is then the concatenated vector of the $v_i$'s, scaled by the side lengths:
...
{\color{blue}Normed space is required for fast approximate NN search techniques such as LSH.} \emph{is that tree also for kdtree}
