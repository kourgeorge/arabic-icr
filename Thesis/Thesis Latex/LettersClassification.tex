...
In fact, MPEG-7 has set several principles to measure a shape descriptor, that is, good retrieval accuracy, compact features, general application, low computation complexity, robust retrieval performance and hierarchical coarse to fine representation.
   
For a detailed review on shape descriptors see \cite{zhang2004review} and \cite{yang2008survey}.

Many types of shape descriptors have been proposed to improve the segmentation and recognition rates. Shape descriptors can be classified into two: 1. local features that are computed at each sample point, i.e. simple: tangent slope angle
normalized curvature and ratio of tangents (invariant). 
Complex: which are invariant under translation rotation and scaling.
such as: shape context and MAD Descriptor. 
2. High-level features, i.e. cusps, crossings and loops. 

\subsection{Local features}

\begin{itemize}
\item Writing direction.
\item Curvature
...
\emph{talk about hierarchical feature and mention the following:}
{\color{blue} 
...
List of descriptors to mention:
\begin{itemize}
\item projection profile
\item upper and lower profile
\item bending
\end{itemize}
...
Many features operate on the contour of the shape. This is because most descriptors were developed for off-line case which has a contour thus shape descriptors are more adequate. In the on-line handwriting recognition, the input data is a sequence of points in the 2-D plane. 

The following is taken from "Combining high-level features with sequential local features for on-line handwriting recognition" \cite{hu1997combining}
\begin{itemize}
\item Local features that are computed at each sample point. 
\item The features are: the y coordinate, tangent slope angle
normalized curvature and ratio of tangents (invariant)
\item these local features are commonly used in many on-line handwriting recognition systems.
\item two more complex features that are invariant under translation rotation and scaling described in "invariant features for HMM based recognition" and in "HMM based on-line handwriting recognition". The incorporation of these two features which are independent of the size, location and orientation of the input word greatly improves the robustness of the system in dealing with unconstrained handwriting, where normalization does not always yield desired results.
\item then hight-level features are then extracted from the script: cusps, crossings and loops. cusps are pints of abrupt change of drawing direction. The smoothing in the preprocessing should remove such wiggles while preserving the dominant direction changes.
\item cusps extraction is done by examining the angle formed by the line segments connecting three consecutive sample points. If this angle is smaller than a preset threshold, the center point is labeled as a cusp.
\item crossing occurs when two separated segments intersect. Two segmented along the script are considered separated if the distance between them them along the drawing trajectory is larger than a threshold. when such intersection is detected, the endpoint closer to the intersection on each intersecting segment is labeled as a crossing point.
\item to find all crossings, an input script containing $n$ points is sequentially searched for pairs of end points of separated segments which are located nearest each other in the x-y space. 
\item The segment between to crossing points is declared a loop if the is no pen-up in between.
...
\begin{itemize}
\item shape retrieval: searching for all shapes in a typically large database of shapes that are similar to a query shape. Usually all shapes within a given distance from the query are determined or the first few shapes that have the smallest distance. 
\item shape recognition and classification: determining whether a given shape matches a model sufficiently, or which of representative class is the most similar. 
\item shape alignment and registration: transforming or translating one shape so that it best matches another shape, in whole or in part. 
\item shape approximation and simplification: constructing a shape with fewer elements (points, segments, triangles, etc.), so that it is still similar to the original. 
\end{itemize}

Many shape description and similarity measurement techniques have been developed in the past. A number of new techniques have been proposed in recent years. There are 3 main classification methods as follows: 
\begin{itemize}
\item contour-based methods and region-based methods [1]. This is the most common and general classification and it is proposed by MPEG-7. It is based on the use of shape boundary points as opposed to shape interior points. Under each class, different methods are further divided into structural approaches and global approaches. This sub-class is based on whether the shape is represented as a whole or represented by segments/sections (primitives). 
\item space domain and transform domain [2]. Methods in space domain match shapes on point (or point feature) basis, while feature domain techniques match shapes on feature (vector) basis. 
\item information preserving (IP) and non-information preserving (NIP). IP methods allow an accurate reconstruction of a shape from its descriptor, while NIP methods are only capable of partial ambiguous reconstruction. For object recognition purpose, IP is not a requirement.
\end{itemize} 

Unlike the traditional classification, the approaches of shape-based feature extraction and representation are classified according to their processing approaches: One-dimensional function for shape representation, Polygonal approximation, Spatial interrelation feature, 
Moments, Scale space approaches, Shape transform domains. The figure 1 shows the hierarchy of the classification of shape feature extraction approaches. 
}
