...
However, the Euclidian distance has some disadvantages in regards to curve sequences. First, it requires the two sequences to be of the same length. Second, it does not support local shifting. 
...
The most important property of a metric function is triangularity, namely the triangle inequality.
One way of interpreting triangularity is that the concept of overlap becomes meaningful for naturally defined regions such as metric balls. 
If we're allowed to violate triangularity, two seemingly non-overlapping balls could still share objects. See figure \ref{fig:non_triangular_space} \cite{chen2005similarity}.
