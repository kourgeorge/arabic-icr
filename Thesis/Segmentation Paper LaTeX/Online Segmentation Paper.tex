...
\label{sec:results}
Our system was implemented in Matlab environment. The total number of samples in the test set is 320. As mentioned before, each sample is city name. It may contain one or more words. Comparing our system results with other works done in this area is not a simple task. The complexity of such task is the different experimental settings and databases and methodology. However, it is apparent that the results are promising. To give the reader a good perception of the experimental results in this work we show our results on different levels and different aspects of our results. The large test sample and the usage of the ADAB database and not a own collected database give our results further firmness.

To help the reader better understand the system results in figure \ref{fig:sample_parts} we show the different parts of a sample. 
...
In table \ref{} we show statistics of our testing sample set.
...
\emph{1.Show how many segmentation points were found in the nomination stage. We would like to show that every real segmentation point was found at least one time in the nomination stage. 2.Effectiveness and accuracy of the filtering stage. show how many segmentation point were filtered out in the filtering.}
...
The segmentation points results were validated automatically, if a word part was recognized correctly that means that the segmentation is correct. For each letter we select the 3 letter and show our recognition results based on if one of the candidate letters is the correct letter.
