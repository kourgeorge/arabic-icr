...
\begin{abstract}
the cursive and unconstrained nature of the Arabic script, both printed and handwritten, anneals the task of segmentation. Real time performance is necessary in applications involving on-line handwriting recognition. However, most techniques for handwriting recognition wait until the entire curve is traced out before starting the analysis which will inevitably cause delays after the character is written. This paper proposes a novel ongoing on-line strokes-level recognition-based segmentation technique of the Arabic script. The advantage of our approach is that the most time consuming procedures are performed while the stroke is being scribed. The system has been designed and tested using the ADAB Database and promising results were obtained.
\end{abstract}
\begin{IEEEkeywords}
...
\date
\IEEEPARstart{H}{andwriting} remains the most used mean of communication and recording of information in the daily life. Therefore, a growing interest in the handwriting character recognition field has taken place in recent years. Handwriting recognition can be categorized into two main fields: off-line and on-line. In the off-line, a digital image containing text is fed to the computer and the system attempts to convert the spatial representation of the letters into digital symbols \cite{al2011online}. On the contrary, in the on-line case, the recognition process is done on the digital representation of the text written on a special digitizer, PDA, tablet or smart-phone devices where sensors picks up the pen-tip movements.\\
